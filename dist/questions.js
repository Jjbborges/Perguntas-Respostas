...
async function main() {
    ...
    let usuarioId;
    try {
        const result = await pool.query('INSERT INTO usuarios (nome) VALUES ($1) RETURNING id', [nome]);
    ...
    try {
        ...
        if (err instanceof Error)
            ...
            console.error('Erro desconhecido ao buscar perguntas.');
        rl.close();
        return;
    }
    let pontuacao = 0;
    ...
    try {
        ...
        if (err instanceof Error)
            ...
            console.error('Erro desconhecido ao salvar pontuação.');
    }
    rl.close();
    process.exit(0);
}
